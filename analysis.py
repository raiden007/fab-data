...
def create_graphs(matches, round1_matches=None, matches_by_round=None, all_rounds=None):
    ...
    summary_html = (
        f"<h2>Total unique players (from Round 1): {total_unique_players}</h2>"
    )

    # Deck count table
    sorted_decks = deck_counts.most_common()
    deck_table_fig = go.Figure(
        data=[
            go.Table(
                header=dict(values=["Deck", "Player Count"]),
                cells=dict(
                    values=[
                        [deck for deck, _ in sorted_decks],
                        [count for _, count in sorted_decks],
                    ]
                ),
            )
        ]
    ...
    deck_bar_fig = go.Figure(
        [
            go.Bar(
                x=[deck for deck, _ in sorted_decks],
                y=[count for _, count in sorted_decks],
                text=[count for _, count in sorted_decks],
                textposition="inside",
            )
        ]
    )
    deck_bar_fig.update_layout(
        title="Deck Distribution (Round 1 only)",
        xaxis_title="Deck",
    ...
    day2_players = [
        name for name, wins in player_win_counts.items() if wins >= win_cutoff
    ]
    ...
    romanian_players = [
        name for name, country in player_countries.items() if country.upper() == "RO"
    ]
    rom_table_rows = []
    for name in romanian_players:
        wins = player_win_counts.get(name, 0)
        winrate = (
            f"{(wins/player_match_counts[name]*100):.1f}%"
            if player_match_counts[name]
            else "0.0%"
        )
    ...
    rom_table_fig = go.Figure(
        data=[
            go.Table(
                header=dict(values=rom_headers),
                cells=(
                    dict(values=[list(col) for col in zip(*rom_table_rows)])
                    if rom_table_rows
                    else dict(values=[[], [], [], []])
                ),
            )
    ...
    rom_table_fig.update_layout(
        title="Romanian Players and Their Win Rate (All Rounds, Sorted by Wins)"
    )
            ...
            table_fig = go.Figure(
                data=[
                    go.Table(
                        header=dict(values=headers),
                        cells=(
                            dict(values=[list(col) for col in zip(*rows)])
                            if rows
                            else dict(values=[[], [], [], [], [], [], []])
                        ),
                    )
